...
The system will ignore the spatial index if the spatial conditions used specify it must be ignored, for example with the text matching conditions.  It can also be turned off (deleted) if needed by using the _Analyses->Index_ menu.  From version 1.0, users can also set the index to be ignored on a per-condition basis.  This means it can be used for the first condition, but ignored for the second, or vice versa.  This is controlled through the options at the right of the conditions (near the big green tick icon).
...
In some cases the index can result in much longer analysis times, for example where the BaseData groups are very sparsely distributed.  In these cases most of the index offsets are empty, but the system must repeatedly search through them to find the actual elements.  One might, for example, have 500 Basedata groups and a spatial condition which spans a large area and thus searches through 10,000 or more index elements to find the neighbours for each group.  In such cases it is better to ignore the spatial index.  See [issue #545](https://github.com/shawnlaffan/biodiverse/issues/545) for more details.
...
It is also worth noting that the spatial analyses are optimised to skip searching for neighbours in cases where an analysis is using the same spatial conditions and definition query as another spatial analysis in that BaseData.  In such cases, the analysis copies the neighbour sets across from the other analysis.  This can substantially increase the speed of any analyses, but will obviously affect any comparison between index size experiments suggested above.  In such cases you should delete the analysis output before re-running with a different index configuration.
...
See [this image](https://purl.org/biodiverse/wiki/screenshots/cluster_analysis_results_page_recalc_linkage.png) for an example.

If the negative nodes make interpretation difficult then change the display [to plot by depth](https://purl.org/biodiverse/wiki/screenshots/cluster_analysis_results_page_recalc_linkage_plot_by_depth.png).
